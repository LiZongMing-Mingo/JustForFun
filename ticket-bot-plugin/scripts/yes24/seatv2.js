...
// 创建Toast样式
const toastStyles = `
<style id="ticket-bot-toast-styles">
    .ticket-bot-toast-container {
        position: fixed;
        top: 20px;
        right: 20px;
        z-index: 999999;
    ...
    .ticket-bot-toast.show {
        opacity: 1;
        transform: translateX(0);
    }
    
    .ticket-bot-toast.success {
        background: linear-gradient(135deg, #4CAF50, #45a049);
        border-left: 4px solid #2E7D32;
    }
    
    .ticket-bot-toast.error {
        background: linear-gradient(135deg, #f44336, #d32f2f);
        border-left: 4px solid #C62828;
    }
    
    .ticket-bot-toast.info {
        background: linear-gradient(135deg, #2196F3, #1976D2);
        border-left: 4px solid #1565C0;
    }
    
    .ticket-bot-toast.warning {
        background: linear-gradient(135deg, #FF9800, #F57C00);
        border-left: 4px solid #E65100;
    }
    
    .ticket-bot-toast-title {
        font-weight: bold;
        margin-bottom: 4px;
    }
    
    .ticket-bot-toast-message {
        font-size: 12px;
        opacity: 0.9;
    }
</style>
...
function initToastSystem() {
    if (!document.getElementById('ticket-bot-toast-styles')) {
        document.head.insertAdjacentHTML('beforeend', toastStyles);
    }
    
    if (!document.getElementById('ticket-bot-toast-container')) {
        const container = document.createElement('div');
        container.id = 'ticket-bot-toast-container';
        container.className = 'ticket-bot-toast-container';
...
function showToast(title, message = '', type = 'info', duration = 5000) {
    ...
    const container = document.getElementById('ticket-bot-toast-container');
    const toast = document.createElement('div');
    toast.className = `ticket-bot-toast ${type}`;
    
    toast.innerHTML = `
        <div class="ticket-bot-toast-title">${title}</div>
        ${message ? `<div class="ticket-bot-toast-message">${message}</div>` : ''}
    `;
...
function toastError(title, message, duration = 7000) {
...
function toastWarning(title, message, duration = 6000) {
    showToast(title, message, 'warning', duration);
}

/*--------------------------------- 自定义配置 USERID必填 ---------------------------------*/
let seatSelect = []; // 没用 todo:增加自定义选座
let blockSelect = [314,315,310,309,319,320,304,305]; // 默认自定义选区（如果用户未配置）
let SEAT_MAX_CLICK_COUNT = 30; // 单个座位最大点击次数
let WEBHOOK_URL = ''; // 飞书webhook url
let USERID = 'N20250625134525146'; // 用户id 抓包自己看
...
// 去重集合 - 防止重复添加相同座位
let processedSeats = new Set();  // 存储已处理的座位ID
let secondSeatSet = new Set();   // 存储二次队列中的座位ID，防止重复
...
async function selectDate(data) {
    ...
    let time = data.time;
    if (date) {
        toastInfo('📅 选择日期', `正在选择日期: ${date}`);
        document.getElementById(date).click();
        await sleep(500);
        if (time) {
            toastInfo('⏰ 选择时间', `正在选择时间: ${time}`);
...
// 监听Tampermonkey脚本的消息反馈
window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'tampermonkey-success') {
        toastInfo('🔧 辅助脚本', 'Tampermonkey脚本执行成功');
    }
});
...
function getUserInfo() {
    toastInfo('🔄 初始化', '开始获取用户信息...');

    const iframe = document.querySelector('iframe');
    if (!iframe) {
        toastError('❌ 初始化失败', '未找到iframe元素');
        return;
    }

    toastInfo('✅ 页面检测', '成功找到票务iframe');
    ...
    try {
        ...
        toastSuccess('🎯 配置成功', `场次ID: ${idTime}, 场馆ID: ${idHall}`);
        ...
        toastError('❌ 配置失败', '解析iframe URL失败');
        ...
        seats.forEach(seatData => {
            if (seatData.trim()) {
                ...
                if (seatInfo.length > 0) {
                    ...
                    if (!isSuccess) {
                        // 创建唯一标识符
                        let seatKey = `${block}-${seatId}`;
                        
                        if (chooseable == "0") {
                            // 空座位 - 立即处理，不需要去重检查
                            toastSuccess('🎯 发现空座!', `区域${block} 座位${seatId} 已加入锁定队列`);
                            addSeatToQueue(seat);
                            sendFeiShuMsg(WEBHOOK_URL,`刷到空座，直接接口锁定 block:${block} seat:${seatId} index:${seatElementIdx},seatInfo:${seatInfo}`)
                        }else{
                            // 可能座位 - 需要去重检查
                            if (!secondSeatSet.has(seatKey)) {
                                toastInfo('🔍 发现可能座位', `区域${block} 座位${seatId} 加入尝试队列`);
                                secondSeatQueue.push(seat);
                                secondSeatSet.add(seatKey);  // 记录已添加
                            }
                            // 如果已存在则静默跳过，避免重复添加
                        }
                    }
...
async function searchSeat() {
    ...
    while (!isSuccess) {
        ...
        // 一直循环遍历blockSelect
        let currentBlock = blockSelect[i];
        stats.searchCount++;
        
        if (stats.searchCount % 50 === 0) {
            let runtime = Math.floor((Date.now() - stats.startTime) / 1000);
            toastInfo('📊 搜索统计', `已搜索${stats.searchCount}次 | 运行${runtime}秒 | 主队列:${seatQueue.length} 备选:${secondSeatQueue.length}`);
        }
        
        sendSearchSeatRequest(currentBlock);
...
async function trySecondSeat(){
    if (seatQueue.length == 0 && secondSeatQueue.length > 0 && !isSuccess) {
        ...
        if (seat) {
            // 从去重集合中移除
            let seatKey = `${seat.block}-${seat.id}`;
            secondSeatSet.delete(seatKey);
            
            toastWarning('🔄 尝试可能座位', `区域${seat.block} 座位${seat.id} (备选队列)`);
...
async function lockSeat() {
    toastInfo('🚀 插件启动', '开始自动抢票流程...');
    let concertId = getConcertId();
    let data = await get_stored_value(concertId);
    
    // 使用用户配置的section，如果没有则使用默认的blockSelect
    if (data && data.section && Array.isArray(data.section) && data.section.length > 0) {
        // 将用户输入的section转换为数字数组
        blockSelect = data.section.map(s => parseInt(s.toString().trim())).filter(n => !isNaN(n));
        toastSuccess('✅ 用户配置', `使用自定义区域: ${blockSelect.join(', ')}`);
    } else {
        toastInfo('ℹ️ 默认配置', `使用默认区域: ${blockSelect.join(', ')}`);
    }
    ...
    while (!isSuccess) {
        if (seatQueue.length > 0) {
            ...
            if (seat) {
                stats.lockAttempts++;
                toastWarning('🔒 尝试锁定', `区域${seat.block} 座位${seat.id} (第${stats.lockAttempts}次尝试)`);
                sendSeatLockRequest(seat.block,seat.id,true)
            }
            popSeatFromQueue();
            await sleep(400)
    ...
    // 接口锁成功的处理一下选座
    toastSuccess('🎉 抢票成功!', `区域${successBlock} 座位${successId} 已锁定`, 10000);
    sendFeiShuMsg(WEBHOOK_URL, `抢票成功 successBlock:${successBlock} successId:${successId}`);
    
    toastInfo('💳 进入支付', '正在进入支付流程...');
